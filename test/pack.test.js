...
run([
    ['the tarball installs and every subpath loads (CJS and ESM)', async () => {
        ...
        try {
            ...
            for (const must of ['package.json', 'index.js', 'browser.js', 'LICENSE', 'README.md', 'CHANGELOG.md', 'src/core/client.js', 'esm/core.mjs', 'types/core.d.ts', 'types/contracts.d.ts']) {
            ...
            const subpaths = Object.keys(pkg.exports).filter((s) => s !== './package.json').map((s) => (s === '.' ? 'openvibe-sdk' : `openvibe-sdk/${s.slice(2)}`));
            const script = `
                const assert = require('node:assert/strict');
                ...
                (async () => {
                    ...
                    console.log('consumer ok: ' + specs.length + ' subpaths');
                })().catch((err) => { console.error(err); process.exit(1); });`;
            const res = execFileSync(process.execPath, ['-e', script], { cwd: app, encoding: 'utf8' });
            assert.match(res, /consumer ok: 12 subpaths/);
