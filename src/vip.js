...
/**
 * openvibe-sdk/vip — the consumer seam for products that honour OpenVibe.VIP memberships (Live, Chat,
 * Community, Blog, Wiki, …): VIP's openvibe-vip/client, published once here instead of being copied
 * into every product. Server-side (it sends a service token); no dependencies beyond fetch.
 *
 *   const { createVipClient, createVipCache } = require('openvibe-sdk/vip');
 *   const tokens = createServiceTokenClient({ tokenUrl, clientId, clientSecret, audience: 'openvibe.vip' });
 ...
 *
 * tokenClient is anything with authHeaders() (and optionally invalidate()): openvibe-sdk/auth's
 * createServiceTokenClient or openvibe-contracts serviceAuth.createTokenClient.
 *
